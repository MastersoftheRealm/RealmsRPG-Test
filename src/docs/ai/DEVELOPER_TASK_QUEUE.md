...
| **DEV-007** | [TASK-649](ACTIVE_TASKS.md) — Supabase least-privilege Phase 2 | Review five proposed SQL files in `sql/task-649-*.sql` (indexed in `sql/README.md` § Proposed). Confirm guided-creator enrichment is stable (backup rollback not needed). Optional: `pg_dump` backup tables before drop. Reply **apply** to apply in order 1→5 via Supabase MCP or Dashboard. Post-apply: re-run advisors; smoke-test codex read, codex-art image URLs, login/signup, character sheet, campaigns; update `SUPABASE_SCHEMA.md`. Unused-index drops (Part B) deferred — decide after traffic review. | Live DB mutation + owner sign-off | | | | |
