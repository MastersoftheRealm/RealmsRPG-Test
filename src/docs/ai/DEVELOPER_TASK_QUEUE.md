...
## Owner checklist — signup CAPTCHA (TASK-899 / DEV-015)

**Safe today:** Until you complete this checklist, production behaves exactly as before — no Turnstile widget, no CAPTCHA token required. Code is ready; protection is **off** until keys + Dashboard toggle.

**Do not enable Supabase CAPTCHA before step 2 is deployed** — that would block all signups/logins.

| Step | Where | Action |
| ---- | ----- | ------ |
| 1 | [Cloudflare Turnstile](https://dash.cloudflare.com/?to=/:account/turnstile) | Add site **RealmsRPG**. Domains: `realmsrpg.com`, `www.realmsrpg.com`, `localhost`. Widget: **Managed**. Copy **site key** + **secret key**. |
| 2 | [Vercel](https://vercel.com) → project **realms-rpg-test** → Settings → Environment Variables | Add `NEXT_PUBLIC_TURNSTILE_SITE_KEY` = site key. Environments: **Production** + **Preview**. **Redeploy production.** |
| 3 | [Supabase Dashboard](https://supabase.com/dashboard/project/lbqhiwudvifmkjtkccdg/auth/protection) → Authentication → **Bot and Abuse Protection** | Enable CAPTCHA → provider **Turnstile** → paste **secret key** (not the site key). |
| 4 | Same Supabase Auth settings | **Rate Limits** — lower signup / email-send thresholds. |
| 5 | Supabase → Authentication → **Password** | Minimum length **≥ 8** (optional: match in app later). Enable **Leaked password protection** (**DEV-001**). |
| 6 | Production | Verify `/register`, `/login`, `/forgot-password` show Turnstile and still work. Verify `https://realmsrpg.com/.well-known/security.txt`. Run **DEV-V-007 T006–T007** in `BUILD_VALIDATION.md`. |
| 7 | Optional | Reply to security reporter: “Thanks — CAPTCHA + security.txt enabled.” |

Full detail: `src/docs/DEPLOYMENT_AND_SECRETS_SUPABASE.md` § Auth CAPTCHA.

---

## API route auth/IDOR test coverage (TASK-658 / TASK-713 / TASK-860)

Vitest auth/IDOR suites cover **characters**, **campaigns** (`route.test.ts`, `[id]/route.test.ts`, `[id]/characters/...` — TASK-776 encounter vs full RM; **TASK-860** invite `[code]` + `[id]/rolls`), **admin** (users list; **TASK-860** check / changelogs / role-policies / update-role), **user library**, **user enhanced items**, **encounters**, **crafting**, **images** (public GET; admin POST/PATCH/DELETE/replace/usage), **uploads** (portrait owner IDOR; profile-picture role gate), **official/[type]** (public GET; admin POST/DELETE), **official/enhanced-items** (admin-only including GET), and **codex** (public GET; production `?debug=1` does not leak to anonymous callers). Tests are co-located `route.test.ts` next to each route.

Human campaign/admin/storage smoke remains **DEV-V-002/003/004**. HIBP stays TASK-326/353 (WAITING, Dashboard).
...
| 2 | DEV-V-016-T003 — Per-kind armament load columns | **CI** — `library-selectable-builders.test.ts` (weapon Damage / armor DR / shield Block+Damage; no type/stat mixed headers) |
...
**Already automated (pre-TASK-480):** DEV-V-014 (`codex-payload` + roll timestamp); large guided unit suite (e.g. `src/lib/guided-creator/build-character.test.ts`); Playwright visual/a11y + guided audits.
...
| **DEV-014** | [TASK-834](WAITING_TASKS.md) — recovery spot-check | After the 2026-08-18 `git checkout -- src`, compare OneDrive versions (before 18:50) for library tabs, creature workspace, and docs that transcript replay did not match. Restore only files that are still stale. Content recovery only — does not gate TASK-824 dest-type retry on current disk. | Needs OneDrive version history |
| **DEV-015** | [TASK-899](ACTIVE_TASKS.md) — Auth CAPTCHA + rate limits | **After** frontend deploy with Turnstile widget: (1) Cloudflare Turnstile site for `realmsrpg.com` (+ localhost). (2) Vercel `NEXT_PUBLIC_TURNSTILE_SITE_KEY`. (3) Supabase → **Authentication** → **Bot and Abuse Protection** → enable CAPTCHA (Turnstile secret). (4) Tighten Auth rate limits + min password ≥8. (5) Enable HIBP (**DEV-001**). Project `lbqhiwudvifmkjtkccdg`. See `DEPLOYMENT_AND_SECRETS_SUPABASE.md`. | Dashboard + Cloudflare + Vercel; MCP cannot toggle |
...
Each suite is a **category** of step-by-step tests. Full steps for Ready / Pending QA suites live in `[BUILD_VALIDATION.md](BUILD_VALIDATION.md)`. Archived suites live in [`archive/BUILD_VALIDATION_ARCHIVE.md`](archive/BUILD_VALIDATION_ARCHIVE.md). Do not run cramped multi-behavior checklists — use individual **DEV-V-###-T###** tests.


| Suite         | Category                                              | Related task(s)                                                                                                                                                          | Tests             | Status                                                                                                            |
| ------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | ----------------------------------------------------------------------------------------------------------------- |
| **DEV-V-001** | Advanced character creator step guards                | TASK-356, TASK-717, TASK-748, TASK-739, **TASK-804**, **TASK-848**, **TASK-856**                                                                                                                         | T001–T021 (21)    | Ready — [open suite](BUILD_VALIDATION.md#dev-v-001--advanced-character-creator-step-guards)                       |
| **DEV-V-008** | Archetype path completion                             | TASK-366–374, TASK-473, TASK-484, TASK-404, TASK-476, TASK-512, TASK-514–518, TASK-522, TASK-529, TASK-534, TASK-572, TASK-381, TASK-732, **TASK-788**                                                                     | T001–T026         | Ready — [open suite](BUILD_VALIDATION.md#dev-v-008--archetype-path-completion-task-366374)                        |
| **DEV-V-009** | Character sheet refactor                              | TASK-317, TASK-348, TASK-365, TASK-375, TASK-318, TASK-349, TASK-483, TASK-485, TASK-486, TASK-502, TASK-478, TASK-508–513, TASK-523, TASK-525, TASK-526, TASK-537, TASK-538, TASK-542, TASK-543, TASK-546, TASK-547, TASK-567, TASK-582, TASK-581, TASK-583, TASK-584, TASK-585, TASK-586, TASK-587, TASK-594, TASK-600, TASK-602, TASK-667, TASK-733, TASK-736, TASK-741, TASK-747, TASK-750, TASK-761, **TASK-773**, **TASK-778**, **TASK-779**, **TASK-780**, **TASK-782**, **TASK-783**, **TASK-786**, **TASK-787**, **TASK-788**, **TASK-800**, **TASK-803**, **TASK-805**, **TASK-815**, **TASK-799**, **TASK-838**, **TASK-837**, **TASK-839**, **TASK-843**, **TASK-835**, **TASK-836**, **TASK-868**, **TASK-894**, **TASK-902**, **TASK-903** | T001–T076         | Ready — [open suite](BUILD_VALIDATION.md#dev-v-009--character-sheet-refactor-task-317-task-348-task-365-task-375-task-483-task-485-task-486-task-502-task-478) |
| **DEV-V-005** | RLS / DB migrations                                   | TASK-352, TASK-327, TASK-354                                                                                                                                             | T001–T003 (3)     | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-005--rls-policy-consolidation-task-352-task-327) |
| **DEV-V-010** | Feat/trait custom name + note                         | TASK-377                                                                                                                                                                 | T001–T004 (4)     | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-010--feattrait-custom-name--note-task-377)       |
| **DEV-V-011** | UI verification safety net                            | TASK-383, TASK-385                                                                                                                                                       | T001–T006 (6)     | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-011--ui-verification-safety-net-task-383)        |
| **DEV-V-012** | Landing page rebuild                                  | TASK-387, TASK-519, **TASK-763**, **TASK-789**, **TASK-833**                                                                                                                              | T001–T010 (10)    | Ready — [open suite](BUILD_VALIDATION.md#dev-v-012--landing-page-rebuild-task-387)                                |
| **DEV-V-013** | Guided Simple character creator                       | TASK-394–403, TASK-406–407, TASK-419, TASK-422, TASK-424–429, TASK-432–436, TASK-441–443, TASK-446–448, TASK-451–453, TASK-454–462, TASK-444, TASK-463–468, TASK-470–472, TASK-487–490, TASK-503, TASK-520, TASK-514, TASK-524, TASK-527, TASK-528, TASK-530, TASK-544, TASK-545, TASK-547, TASK-548, TASK-565, TASK-566, TASK-573, TASK-577, TASK-578, TASK-579, TASK-580, TASK-638, TASK-640, TASK-641, TASK-670, TASK-711, TASK-706, TASK-707, TASK-730, TASK-720, TASK-729, TASK-726, TASK-716, TASK-732, TASK-734, **TASK-755**, **TASK-756**, **TASK-757**, **TASK-758**, **TASK-759**, **TASK-760**, **TASK-753**, **TASK-790**, **TASK-798**, **TASK-820**, **TASK-829**, **TASK-848**, **TASK-872** | T001–T092 | Ready — [open suite](BUILD_VALIDATION.md#dev-v-013--guided-simple-character-creator-task-394403)                  |
| **DEV-V-014** | Codex payload + roll timestamp                        | TASK-378                                                                                                                                                                 | T001+ (CI vitest) | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-014--codex-payload--roll-timestamp-task-378)     |
| **DEV-V-015** | Library API typing                                    | TASK-420                                                                                                                                                                 | T001–T002         | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-015--library-api-typing-task-420)                |
| **DEV-V-031** | API route smoke                                       | TASK-613                                                                                                                                                                 | T001 (CI vitest)  | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-031--api-route-smoke-task-613)                   |
| **DEV-V-016** | Library add/load selection parity + GridListRow facts | TASK-379, TASK-437, TASK-475, TASK-536, TASK-541, TASK-564, TASK-574, TASK-480, TASK-675, TASK-691, TASK-712, TASK-723, **TASK-774**, **TASK-807**, **TASK-808**, **TASK-809**, **TASK-810**, **TASK-812**, **TASK-813**, **TASK-814**, **TASK-815**, **TASK-818**, **TASK-825**, **TASK-868**, **TASK-870**                                                                                         | T001–T030 (+ CI builders/inventory) | Ready — [open suite](BUILD_VALIDATION.md#dev-v-016--library-addload-selection-parity-task-379)                    |
| **DEV-V-017** | Site copy modules                                     | TASK-390, TASK-765                                                                                                                                                       | T001–T006 (6)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-017--site-copy-modules-task-390)                                   |
| **DEV-V-018** | CreatorPageShell parity                               | TASK-380, TASK-431, TASK-381, TASK-615, TASK-616, TASK-732, **TASK-764**, **TASK-408**, **TASK-832**, **TASK-826**, **TASK-828**, **TASK-866**                                                                                                                     | T001–T018 (18)    | Ready — [open suite](BUILD_VALIDATION.md#dev-v-018--creatorpageshell-parity-task-380--task-381--task-616)                             |
...
| **DEV-V-020** | Sitewide copy compliance                              | TASK-439, TASK-440, **TASK-784**, **TASK-872**                                                                                                                                                       | T001–T005 (5)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-020--sitewide-copy-compliance-task-439)                            |
...
| **DEV-V-022** | Characters list page                                  | TASK-469                                                                                                                                                                 | T001–T003 (3)     | Archived — [open suite](archive/BUILD_VALIDATION_ARCHIVE.md#dev-v-022--characters-list-page-task-469)              |
...
| **DEV-V-028** | Codex browse list shell                               | TASK-576, **TASK-799**, **TASK-827**, **TASK-840**, **TASK-842**, **TASK-845**, **TASK-849**, **TASK-852**                                                                                                                            | T001–T007 (7)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-028--codex-browse-list-shell-task-576)                            |
| **DEV-V-030** | Encounter play facades                                | TASK-608, TASK-819, **TASK-846**                                                                                                                                          | T001–T004 (4)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-030--encounter-play-facades-task-608) |
...
| **DEV-V-043** | Wave 5 page facade splits                             | TASK-666 / **TASK-762**                                                                                                                                                  | T001–T008 (8)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-043--wave-5-page-facade-splits-task-666)                         |
| **DEV-V-042** | Campaigns RLS + create                                | TASK-650 / **TASK-802**                                                                                                                                                  | T001–T003 (3)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-042--campaigns-rls-select-consolidation-task-650--task-802)     |
| **DEV-V-045** | Codex character filter UX                              | Session / TASK-681 / TASK-722 / **TASK-781**                                                                                                                              | T001–T004 (4)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-045--codex-character-filter-ux-session)                            |
...
| **DEV-V-051** | Guided funnel entry, trusted create, feat choice        | **TASK-738** / **TASK-754** / **TASK-804**                                                                                                                                | T001–T011 (11)    | Ready — [open suite](BUILD_VALIDATION.md#dev-v-051--guided-funnel-entry-trusted-create-feat-choice-task-738--task-754) |
...
| **DEV-V-053** | Wave 3A SEO + token hygiene                             | **TASK-769** / **TASK-770** / **TASK-771** / **TASK-793** / **TASK-853**                                                                                                    | T001–T006 (6)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-053--wave-3a-seo--token-hygiene-task-769--task-770--task-771) |
| **DEV-V-054** | Codex per-collection fetch + virtualized browse rows    | **TASK-775**                                                                                                                                                             | T001–T003 (3)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-054--codex-per-collection-fetch--virtualized-browse-rows-task-775) |
| **DEV-V-055** | ADR-0023 control touch tiers                            | **TASK-841**, **TASK-847**, **TASK-830**, **TASK-850**, **TASK-851**, **TASK-857**, **TASK-865**                                                                                          | T001–T007 (7)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-055--adr-0023-control-touch-tiers-task-841) |
| **DEV-V-056** | Virtual tabletop V1                                    | User request — VTT V1 / TASK-424 / VTT token deletion / VTT monster tokens / VTT character token portraits                                                                 | T001–T011 (11)    | Ready — [open suite](BUILD_VALIDATION.md#dev-v-056--virtual-tabletop-v1) |
| **DEV-V-057** | Encounter duplicate campaign characters                | User request — encounter duplicate campaign characters                                                                                                                     | T001–T003 (3)     | Ready — [open suite](BUILD_VALIDATION.md#dev-v-057--encounter-duplicate-campaign-characters) |
...
| DEV-V-007     | Auth UI (CAPTCHA + Google)                            | TASK-361, **TASK-899**                                                                                                                                                   | T006–T007         | Pending QA after DEV-015                                                                                          |
...
| ---- | ------------- | -------------- |
| **TASK-903** | DEV-V-009 **T076** | Sheet: expand a library row or open a modal → alt-tab away and back → UI still open (no “Loading character…” remount). Optional: different-user sign-in does not leak prior cache. |
| **TASK-902** | DEV-V-009 **T075** (+ **T064**) | Phone sheet: swipe down from header scrolls panel; vertical swipe works on ability/skill/library content (not gutters only); header stays collapsed until panel scrollTop is 0 (no mid-scroll snap-lock); horizontal panel snap still works. 360/390. |
| **TASK-885** | DEV-V-009 **T069** | Header Speed/Evasion/DR/Crit are equal compact/squarish tiles centered in the middle column — not full-bleed bars squeezing identity/resources. Titles fit. 360 / 768 / 1024 / 1280. |
| **TASK-887** | DEV-V-009 **T070** | Sheet Edit Library: hide-eyes stay Dense; typical desktop widths do not force tab scroll solely from eye padding. Hide/show still works; coarse hit ~44px. |
| **TASK-888** | DEV-V-009 **T071** | Feats/Traits Customize is an `outline` Button (not a desc-chip cousin) with even gap chips→rule→button. Desktop + ~360px. |
| **TASK-890** | DEV-V-009 **T072** | Library/Codex tab chevrons only when that edge can scroll; gone when tabs fit. Chevron jumps to first/last tab. |
| **TASK-891** | DEV-V-009 **T073** | Archetype edit: no +/− on prof-point max; remaining/total read-only. Slider thumb centered; dark theme tokenized. Creature Creator slider matches. |
| **TASK-893** | DEV-V-009 **T074** | Hover the roll-log bonus chip: Strength / Reflexes bonus / skill bonus / Attack or ability bonus / Power bonus. Zero modifier has no hover. |
| **TASK-895** | DEV-V-009 **T074** | Outside click/tap closes the roll log; RollButton keeps it open and records; clicks inside the panel do not close; FAB still toggles. No second overlay. |
| **TASK-884** | Spot-check (mixed-species sheet header) | Level 20 mixed-species character at 360/390/768: row 1 is `Level 20` (number never alone on species line); row 2 is both species names together (`Name1 / Name2`, no leading separator). Wide desktop: one line with Level and both names. Single-species still shows one species name. |
| **TASK-882** | DEV-V-013 **T010** (+ sheet/creature Add Skill) | Add Skills modal: Abilities column shows full names (Strength, Agility — not STR chips); guided recommended/path badges do not repeat governing ability on the name; column uses Codex `1.5fr 1fr` tracks and does not crowd the + control. Desktop + ~360px. |
| **TASK-883** | DEV-V-013 **T010** (+ Add Sub-Skill from sheet/creator) | Add Sub-Skills modal: Abilities column shows full comma-separated names via shared `formatSkillAbilityList` (not STR-style abbreviations). Desktop + ~360px. |
| **TASK-889** | Spot-check (sheet + Codex) | Level 2+ feats show **Name II/III…**, not `(Level N)` and not double suffix (e.g. Amplify II at rank 2). Public Codex browse + feat detail page titles match. Add Feat unmet prerequisite says `Requires Speedy II`, not `(Level 2)`. Level-1 names with embedded numerals (Empowered Experience II) unchanged. |
| **TASK-881** | DEV-V-013 **T088** | Guided + Legacy Skills: Defense Bonuses grid uses sheet-style tiles (Defense Score glance, bonus steppers hug value, centered `+N (Nsp)` hint). 2/3/6 grid — not wide slabs. Spend 2 SP on a defense; counter drops. Desktop + ~360px. |
| **TASK-880** | DEV-V-013 **T034–T036** | Guided Abilities recommended + Customize: 2/3/6 grid matches sheet density (text-sm labels, text-xl values, not tall/wide slabs). Customize shows **Ability Points** InfoTippy (max +3, below 0 returns points, floor −2, total negatives ≥ −3). Steppers usable at ~360px. Desktop six-column pills still align. |
| **TASK-899** | DEV-V-007 **T006–T007** (after **DEV-015**) | With Turnstile + Supabase CAPTCHA enabled: `/register` shows widget; Create Account disabled until complete; signup succeeds. Direct API signup without valid token rejected. `/.well-known/security.txt` returns contact + Expires on production. `/login` + `/forgot-password` widgets work when CAPTCHA on. |
| **User request — VTT portrait tokens** | DEV-V-056 **T011** | `/campaigns/[id]/tabletop`: campaign-character token image defaults to the current character portrait; **Sync** backfills missing/stale token images; tokens without images still fall back to initials; RM/player views stay consistent. |
| **TASK-875** | DEV-V-013 **T010** | Add Skills modal footer: **Browse all Sub-Skills** (and Add Sub-Skill **Browse all Skills**) have visible gap below the horizontal separator — not flush on the rule. Guided L2 equipment/powers budget bars in USM footer still clear the rule. Desktop + ~360px. |
| **TASK-898** | DEV-V-016 **T033** (+ **T029** regression) | Sheet Feats edit at 360/390/768/1024: expand row with Uses steppers + Recovery — facts in expanded body only; steppers do not overlap Recovery text. Spot-check power/armament GLR rows with controls. `npm run verify:responsive`. |
| **TASK-897** | DEV-V-009 **T068** | ~360px: Library GLR collapsed names and Archetype weapon names truncate (ellipsis), not one char per line; ability/defense multi-word labels wrap at spaces; header stat labels (Critical Range) wrap at word boundaries. Expand row shows full name. Desktop spot-check. |
| **TASK-896** | DEV-V-009 **T067** | Balanced character (not XP-ready): no toolbar edit dot. Unspent pools only: **green** dot + tooltip lists pools; matching section pencils green where applicable. Overspent only: **red** dot + red section pencil. **Both** over and under across pools: **red/green gradient** dot; tooltip lists both. XP-ready with all pools balanced: **purple** level-up dot. Desktop + ~360px. |
| **TASK-892** | DEV-V-008 **T027** | Pure Martial (2/0) or pure Power (0/2) at level 4: level up to 5 → new prof point auto-assigns to the occupied side (3/0 or 0/3), not left unspent. Powered-Martial 1/1 unchanged. Multi-level jump (4→10) assigns both +1s to pure side. Path admin level-5 floor still applies. Desktop + ~360px. |
| **TASK-894** | DEV-V-009 **T066** | Loaded sheet: Health stepper and typed entry accept negative values (e.g. −3); Energy still floors at 0. Over-heal above max still works. Linked campaign combatant card (editable) allows negative HP; standalone NPC combatant still floors at 0. Desktop + ~360px. |
| **TASK-879** | DEV-V-046 **T009** | Library **Innate Eligible** excludes >1 min, permanent, Adaptation-category, and heal parts; guided **See more Innate Powers** matches; admin path save blocks ineligible innates. Inline saved `part.category` counts. Desktop + ~360px. |
| **TASK-886** | DEV-V-009 **T065** + DEV-V-013 **reveal/finalize** | Guided Reveal + Advanced Finalize: Age is not merged into Appearance text. New character → sheet Notes: Age sits in the physical header after Height/Weight; Backstory section shows creator background; Appearance is appearance-only. Legacy `Age: N` prefix in appearance still shows Age in header; after save, JSON has dedicated `age` and stripped appearance. Desktop + ~360px. |
| **TASK-878** | DEV-V-052 **T006** | Codex/Library/creator/sheet add-X: Archetype Path dropdown lists **Quick select** (Any, Any Power, Any Martial, Any Powered-Martial) first. Guided See more auto-selects one type alias (e.g. Any Martial), not every path chip. Any Power replaces individual Power paths on select; Any replaces whole selection. Desktop + ~360px. |
| **TASK-869** | DEV-V-016 **T032** | Expand a power or technique on sheet Library, Add modal, or Codex browse: Category **column** header unchanged where shown; demoted/expanded descriptor chip is value-only (`Defense`, `Offense, Damage`) — not `Category Defense`. Feat chips still say `Category Utility`. Desktop + ~360px. |
| **TASK-872** | DEV-V-013 **T039** + DEV-V-020 **T005** | Guided Loadout: Equipment phase titles / Browse Equipment / Add all recommended Equipment; no bare Gear or Adventuring Gear. Landing uniqueness + sheet tour Library + Legacy Equipment step + Admin recommended Equipment. Creature/sheet Type `adventuring_gear` → **Equipment**. Desktop + ~360px. |
| **TASK-870** | DEV-V-016 **T030** | Library Weapons/Armor/Shields Currency column vs sheet/Add-modal Currency chips: same market number as item-creator **Currency cost (final)**, not the tiny C sum/multiplier. Codex Equipment still uses stored currency. Desktop + ~360px. Skip Legacy `/characters/new/advanced`. |
| **TASK-868** | DEV-V-009 **T064** + **T021**; DEV-V-016 **T029** | Loaded sheet at 360/390: scroll a panel → sheet header fully leaves (not a leftover half-strip); scroll to top restores it. Swipe Skills/Archetype/Library: each panel lines up with the header card gutters. Feats: expand a row with Uses/Recovery → facts in the body only; empty Uses/Recovery omitted. When expanded, column facts live in the body at all widths (TASK-898); overlap regression → DEV-V-016-T033. |
...
| **TASK-857** | DEV-V-055 **T006** | Sheet Edit/Temp pencils and combatant Dmg/Heal at 390 touch: Dense 32px paint + 44 expanded hit, not a 44 painted slab. Fine-pointer desktop stays compact. Chip dismiss / compact SegmentedControl unchanged in role. Do not retag list thumbs / image mattes / innate-toggle. |
| **TASK-853** | DEV-V-053 **T006** | `/rules` is the MDX chapter index (RulebookNav + chapter links); Google Doc is view-source / open-in-new-tab only (no iframe). Crafting / My Account titles from TASK-793 still hold. Desktop + ~360px. |
| **TASK-825** | DEV-V-016 **T028** | Sheet Library equipment + creature selected Equipment: expand an item with valued TP → Training Points chip; no TP column; no Total TP footer. Codex/Admin/Add equipment stay Category / Currency / Rarity columns; expand may show a TP chip when valued. 0 TP has no chip. Desktop + ~360px. Skip Legacy `/characters/new/advanced`. |
| **TASK-852** | DEV-V-028 **T001** | Admin Codex Duplicate: banner still says “Creating a copy of {source}” with the entity-specific “save to add the new …” line (Skills + spot-check Equipment / Creature Feats / Archetypes). Add/Edit footers unchanged. `/characters/new/advanced` still loads. |
| **TASK-851** | DEV-V-055 **T005** | Header below xl at 390 touch: hamburger is a tappable IconButton square. Fine-pointer ~1100px: hamburger is compact (no min-w 44 slab). Selectable library/Codex row: selection column is not always-on min-w-[44px] w-11; SelectionToggle coarse square remains. Encounter compact Health/Energy (and sibling number fields) Standard 44 under coarse, compact under fine — no md:min-h-0. Do not retag list thumbs / image mattes / innate star. |
| **TASK-850** | DEV-V-055 **T004** | Loaded sheet at 390 touch: ability/skill/defense `RollButton` ≥44px tall. Fine-pointer desktop stays compact (no leftover `min-h` 44 padding). Wide desktop does not shrink via `md:min-h-0`. |
| **TASK-818** | DEV-V-016 **T027** | Path More details: expand a listed power/technique → labeled catalog chips (Energy/Action/Damage/…); Name+Description columns only; no Stats column. Desktop + ~360px. Skip Legacy `/characters/new/advanced`. |
| **TASK-813** | DEV-V-016 **T025** | Library creature expand + Creature Creator Equipment: Qty is blank unless stored (shared `layout="creature"` section, not a fake 1 / second Qty column). Sheet Inventory steppers still default missing qty to 1. Desktop + ~360px. Skip Legacy `/characters/new/advanced`. |
| **TASK-849** | DEV-V-028 **T001** | Admin Codex Skills: Add/Duplicate/Edit still use sticky footer Save / Delete / Cancel; Duplicate name ends with ` copy`. Spot-check Equipment + Creature Feats the same way. `/characters/new/advanced` still loads. |
| **TASK-847** | DEV-V-055 **T002** | Admin Codex spreadsheet at 390 touch vs desktop: toolbar hits follow pointer (not `md:min-h-0`). Library/Codex **Filters** is Standard under coarse, compact under fine (`max-md` 44 gone). Sheet RollLog **send** is shared `Button` (no raw `min-w-[44px]`). Header hamburger / grid selection / combatant compact inputs unchanged. |
| **TASK-848** | DEV-V-001 **T020** + DEV-V-013 **T092** | `/characters/new/advanced` and `/characters/new/guided` at 360/390: overflowing step/chapter rails show fade + chevron; gone when fully scrolled or when everything fits; Restart stays visible on Legacy; page does not side-scroll. `md+` Legacy wrap has no chevrons when steps fit. |
| **TASK-830** | DEV-V-055 **T003** | `/power-creator`, `/item-creator`, `/creature-creator` at 360/390 touch: Name (and Level/Type/Size) fields ≥44px tall, aligned with buttons. Fine-pointer desktop stays compact `h-10`. Codex/Library filter rows still match at `h-11`. |
| **TASK-846** | DEV-V-030 **T004** | Add From Library / Campaign at 390: **Add** stays in the sticky Modal footer on Creature Library and Campaign Characters; quantity + type radios stay in content; still inserts combatants. Non-USM. |
| **TASK-829** | DEV-V-013 **T091** | Guided Path at 360/390: sticky footer and chapter rail are opaque (cards do not show through Continue or the rail). Last card still scrolls above the bar. `md+` frosted look unchanged. |
| **TASK-836** | DEV-V-009 **T063** | Loaded sheet at 360/390 touch: skill proficiency dots stay 16px painted but tappable; Sub-Skills easy to tap; Current Health ≥44px tall. Desktop chrome stays compact. No fifth Skills column. |
| **TASK-835** | DEV-V-009 **T062** | Loaded sheet at 360/390: ability/defense tiles are 2-col; Intelligence / Discernment / Mental Fortitude stay inside their tiles; full GAME_RULES names (no Mental Fort.); tiles in a row share height. Desktop labels stay text-sm. Header name still ellipsizes. |
| **TASK-840** | DEV-V-028 **T007** | `/library` at 768/1024/1280 and sheet Library at 360/390/1024: overflowing tab strips show an edge fade + chevron; affordance gone when fully scrolled; hidden tabs reachable with chevron or arrow keys. `/characters/new/advanced` still loads. |
| **TASK-843** | DEV-V-009 **T061** | RM campaign character view at 360/390: no empty owner-dock strip; dice FAB stays reachable and does not cover the last sheet rows. Owner `/characters/[id]` dock unchanged. |
| **TASK-838** | DEV-V-009 **T058** | Loaded sheet at 360/390: Abilities/Skills/Archetype/Library panels are viewport-bounded (not stretched to Library height); each panel scrolls internally; horizontal snap still works. E2E Baseline Knight. |
| **TASK-837** | DEV-V-009 **T059** | Loaded sheet at 360/390: opaque bottom action dock + dice FAB slot, no FAB-on-FAB; Recovery / Level Up / Add Feat footers fully tappable (docks hidden). md+: toolbar top-right, FAB bottom-right. |
| **TASK-844** | DEV-V-029 **T004** | Sheet tour retake: Next is fully tappable at 360/390 (not under the C4 dock or dice FAB); tour card stays visible. Desktop card still clears the FAB. |
| **TASK-827** | DEV-V-028 **T006** | `/codex` at 360/390: **Advanced** is below the tab strip (not covering Archetypes). Advanced on: Parts/Properties/Creature short labels. md+: Advanced in the bar, no overlap. `/characters/new/advanced` still loads. |
| **TASK-839** | DEV-V-009 **T060** | Loaded sheet (four header stats) at 360 / 390 / 768 / 1024 / 1280 / 1440: equal-track Speed/Evasion/DR/Crit; ≤2 rows; 3-col from `lg`; 1024 header height comparable to 1440; phone cards in-viewport with wrapping labels. E2E Baseline Knight is the fixture. |
| **TASK-828** | DEV-V-018 **T016** | `/creature-creator` at 360/390: STR–CHA ± stay inside their tiles; 2-col on phones; Advanced creator still uses full ability names. |
| **TASK-826** | DEV-V-018 **T017** | `/creature-creator` at 360/390: no horizontal page scroll; closed Roll Log is gone from the a11y tree; open panel fits; FAB stays tappable. |
| **TASK-841** | DEV-V-055 **T001** | Touch 390: styleguide Large ≥48px, default Primary ≥44, Small/Link dense; USM **Add Selected** ≥48px and Cancel ~44; footer links not 167×44 slabs. Fine-pointer chrome stays compact. Creature Creator ± stay inside the tile. `/characters/new/advanced` still loads. |
| **TASK-833** | DEV-V-012 **T010** | `/about` at 360/390: Previous/Next arrows are on-screen and tappable; dice stay ≥44×44. Overlay + `w-full` is on all widths (centered dice row, not the old fixed-width track). |
| **TASK-832** | DEV-V-018 **T015** | Creature Creator Type/Size truncate with an ellipsis (not "Huma"/"Medi"). Codex search placeholder is `Search...` or ellipsized. Desktop + ~360px. |
| **TASK-819** | DEV-V-030 **T003** | Full combatant card Health/Energy have ValueStepper + number inputs; linked-character cards stay read-only. Compact unchanged. |
| **TASK-842** | DEV-V-028 **T005** | Admin Codex: Add/Edit/Duplicate/Save and row trash share `useAdminCodexEntity` chrome (not a third list shell). Footer Save is Primary lg. `/characters/new/advanced` still loads. Desktop + ~360px. |
| **TASK-845** | DEV-V-028 **T005** | Admin Codex: `requestDelete` gone; row trash and edit-modal Delete open DeleteConfirmModal titled with the entity name (not two-click / inline Yes-No). Referenced rows still hit Delete anyway. |
| **TASK-799** | DEV-V-028 **T005** + DEV-V-009 **T057** | Admin Codex: row trash and edit-modal Delete open DeleteConfirmModal (not two-click / inline Yes-No); referenced rows still hit Delete anyway. Sheet: innate star fills/clears; equip circle → check-circle. `/characters/new/advanced` still loads. Desktop + ~360px. |
| **TASK-820** | DEV-V-013 **T090** | Legacy mixed Ancestry + sheet Edit Species: MixedSpeciesSkillPicker TraitSection-style rows (clamp + See more). Guided mixed skills stay GuidedChoiceCard. `/characters/new/advanced` still loads. Desktop + ~360px. |
| **TASK-798** | DEV-V-013 **T089** | Guided Path L3 AbilityPickButton; Guided Species L2 MixedSpeciesModal; portrait crop/library pick; Legacy Ancestry TraitSection + PathHelpCard; sheet Edit Archetype / Edit Species / identity. `/characters/new/advanced` still loads. Desktop + ~360px. |
| **TASK-814** | DEV-V-016 **T026** | Sheet Library expand: power category/range/TP chips; inventory rarity/currency/TP chips (no Cost Nc badge); feat Req. Level/Category/Ability chips. Add Feat: Ability column + Req. Level chip. Official Powers: TP chip, not Total TP footer. Desktop + ~360px. |
| **TASK-815** | DEV-V-009 **T022** (+ DEV-V-016 **T014**) | Sheet Inventory → Add equipment: custom-item form (Name / Notes / Qty / Add custom) is visible under Search without opening Filters. Custom add still stacks/persists. Desktop + ~360px. |
| **TASK-812** | DEV-V-016 **T024** (+ T023) | Creature save/load uses Weapons/Armor/Shields/Equipment buckets (not mixed `armaments[]`). Legacy `data.armaments` still loads. Equipment Qty is `-` unless quantity is stored. Desktop + ~360px. |
| **TASK-810** | DEV-V-016 **T023** (+ T003 / T010) | Armament Load and creature Inventory have no mixed All list. Weapons/Armor/Shields use catalog columns. Creature selected inventory is four lists. Desktop + ~360px. |
| **TASK-809** | DEV-V-016 **T022** (+ T007 / T021) | Add Power / Add Weapon Range chips follow select-density `chipFacts` (not hardcoded). Add Armor shows Crit / Abl. Req. / Agility as chips under a DR column. Desktop + ~360px. |
| **TASK-808** | DEV-V-016 **T022** | Sheet Inventory armor: Crit is a column only (no Critical Range property chip). Add-modal chips do not duplicate columns. Codex feat Ability Requirements stay `Strength 3+` extra-chrome. Desktop + ~360px. |
| **TASK-807** | DEV-V-016 **T021** (+ T007 / T008) | Browse vs play vs Add Power vs Codex gear: Official power comparison columns; sheet Energy is spend `rightSlot`; Add Power Range is an expand chip; Codex/Admin Equipment is Category / Currency / Rarity only (no Damage/DR/Weight fact chips). Desktop + ~360px. |
| **TASK-805** | DEV-V-009 **T056** | Sheet Feats Edit → Customize: typing Custom name / Player note keeps the caret mid-field (no jump to start/end each key). Title/note still save on blur. Two-feat Name sort does not reorder until blur. Traits match. Desktop + ~360px. |
| **TASK-804** | DEV-V-051 **T011** + DEV-V-001 **T019** | Guest **Continue Without Saving** on Guided Your Hero and Legacy Finalize closes the login prompt and stays closed (no flash-reopen). Draft intact; nothing created. Phone: button is in the sticky modal footer. |
| **TASK-803** | DEV-V-009 **T055** | Sheet Skills: names are Codex labels (not ids) after Edit Species; hover/focus on a skill name shows the Codex description. Desktop `lg+` + ~360px. |
| **TASK-802** | DEV-V-042 **T003** (+ T001 verify) | Signed in: `/campaigns` Create succeeds (invite code + View Campaign + list). Console must not block Sentry ingest via CSP. Ignore Grammarly/Iterable/Blackboard. |
| **TASK-800** | DEV-V-009 **T054** (+ T024 / T032 / T034) | Sheet Skills: Edit/Temp open state is named next to the heading; steppers sit in the Value/Temp cell (bonus caption under); Temp strip matches Abilities; no fifth column / side-scroll to discover mode. Desktop `lg+` + ~360px. |
| **TASK-790** | DEV-V-013 **T088** | Guided Skills: Defense Bonuses grid spends Skill Points; multi-ability picker persists to the sheet; Continue still requires remaining 0. Desktop + ~360px. |
| **TASK-789** | DEV-V-012 **T009** | `/` is the (main) landing (no chrome remount to Codex); About and `/characters/new` stay server pages; `?returnTo=` still works on chooser cards. Desktop + ~360px. |
| **TASK-793** | DEV-V-053 **T006** | `/crafting` and `/my-account` have titles; My Account is noindex; `/rules` has crawlable intro plus first-party chapters (iframe removed; index restored in TASK-853). |
| **TASK-788** | DEV-V-009 **T053** (+ DEV-V-008 **T015**) | Play/edit: no DR / Critical Range when unarmored or when equipped armor does not change that stat. Temp mode: both cards show so a temp can be added (unarmored crit = Evasion+10). With DR armor, DR matches the library row. With Critical Range +1 (op 1 level N), header crit = Evasion + 10 + (1+N). Desktop + ~360px. |
| **TASK-787** | DEV-V-009 **T052** (+ T039 footer) | Recovery Cancel / Full Recovery are inset from the modal edge (not flush bottom-right). Spot-check Edit Archetype and one add-X USM for the same gutter, no double padding. Desktop + ~360px. |
| **TASK-783** | DEV-V-009 **T051** | Sheet Feats play view: italic custom name not clipped on the right; expanded row has no View customization / Custom name field / “Note” heading; player note sits in the same description box below the official text, separated by a simple line. Edit still has Customize fields. Traits match. Desktop + ~360px. |
| **TASK-779** | DEV-V-009 **T050** | Sheet Feats traits: Ancestry / Characteristic / Flaw appear only after expand (DescriptorChip, not ExpandableChip); collapsed name has no kind chip; species kind stays omitted; state-feat Archetype/Character badges still on the collapsed name. Desktop + ~360px. |
| **TASK-786** | DEV-V-009 **T049** | Sheet autosave: notes-only edits are PATCH 200 (no same-tab 409 loop); HP then notes both persist; no repeating **Failed to save character** toast. Two-tab T043 still holds. |
| **TASK-775** | DEV-V-054 **T001–T003** | `/codex` browse: each tab requests `/api/codex?collection=<tab>` only (no full payload); long Feats list scrolls smoothly, stays aligned when Filters expand, and expands correctly; admin codex save still shows up on browse and game rules still load. Desktop + ~360px. |
| **TASK-778** | DEV-V-009 **T032** | Sheet Skills: All/Proficient is compact (not SourceFilter chrome); **Sub-Skills** checkbox shows/hides sub-skill rows. Desktop text-hugging; ~360px 44px targets. |
| **TASK-782** | DEV-V-009 **T033 / T034 / T038** (+ T032 step 4) | Toolbar Edit vs Temp are exclusive. Inside Edit, Abilities/Skills open via the section **pencil** (not auto-spend). Inside Temp, header stats + Terminal + Abilities/Skills open via the sliders icon (not all steppers at once). Icon: blue / gold+ / danger−. Campaign view-only has neither. Level-up opens Edit. Desktop + ~360px. |
| **TASK-784** | DEV-V-020 **T002** + DEV-V-013 **T001** | `/characters/new` Custom card: tagline **Fully customizable archetype and Loadout, built step by step.**; bullets explain own archetype type/abilities then species, Feats, Loadout; no Layer 3 / cohesive creator / Forge. Desktop + ~360px. |
| **TASK-781** | DEV-V-045 **T004** | Codex Feats/Skills + Library Powers: Filter by character (i) sits immediately after the title, not at the far right of the row; (i) still works while collapsed. Desktop + ~360px. |
| **TASK-780** | DEV-V-009 **T048** | Sheet edit: no Lvl quantity stepper beside Uses/Recovery. Expand a multi-rank feat — current Feat Levels chip marked; qualified ranks clickable; unqualified disabled; slot count updates. Play view has no rank picker. Desktop + ~360px. |
| **TASK-774** | DEV-V-016 **T020** | `/library` My + Realms: tab badges from counts endpoints; first paint fetches counts + the active tab only; delete updates the badge; guests stay Realms read-only; Enhanced is My-Library only. Desktop + ~360px. |
| **TASK-771** | DEV-V-053 **T001–T004** | `/robots.txt` + `/sitemap.xml` list public routes and disallow /dev /auth /api; homepage OG/twitter + absolute og:image + canonical; `/dev/styleguide`, `/login`, and `/admin` noindex; `/privacy` intro URL is `https://realmsrpg.com`. |
| **TASK-773** | DEV-V-009 **T047** | RM view + other-user public/campaign sheet: library/feats/empowered rows render from GET `enrichment` (no viewer `/api/user/library/*` or `/api/official/*`). Owner sheet still has add-X catalogs. `?scope=encounter` stays HP/EN/AP only. Desktop + ~360px. |
| **TASK-770** | DEV-V-053 **T005** | Dark-theme sheet muted copy still readable after stripping `dark:text-text-secondary` next to `text-text-muted`. Desktop + ~360px. |
| **TASK-762** | DEV-V-043 **T008** | Combat linked campaign characters: HP/EN/AP sync on load via React Query (`?scope=encounter`, not the RM sheet GET); sheet HP in another tab updates combat (realtime or tab-focus refetch); Add Combatant / Add all use the same fetcher; 90s hidden-tab pause still holds. |
...
| **TASK-723** | DEV-V-016 **T008** | Codex + Admin Equipment: Category / Currency / Rarity headers (no Cost, Damage, Dmg. Red.); Currency is a plain number (no “c”, not highlighted). Expand does **not** require Damage/DR/Weight fact chips (gear closed set, TASK-806). Codex Filters: min/max currency; with a character, rarity-this-level and within-currency are off by default. |
...
| **TASK-765** | DEV-V-017 **T005** (+ DEV-006) | /privacy Cookies and Analytics copy; after deploy, Enable Web Analytics in Vercel Dashboard and confirm …/view beacon |
...
| **TASK-602** | DEV-V-009 **T039** (+ **T052**) | Recovery modal: Full/Partial, 2/4/6h, Auto/Manual use SegmentedControl; preview + CTA still work; fullScreenOnMobile; footer inset (TASK-787) |
...
| **TASK-600** | DEV-V-009 **T038** (+ T034) | Header Speed/Evasion: Temp Modifier only — no pencil / permanent base edit (temps appear in Temp mode, not Edit) |
...
| **TASK-584** | DEV-V-009 **T032** | Skills catalog-all base skills; compact All/Proficient + **Sub-Skills** toggle (TASK-778); − clears value→prof (subs remove); no Add Skill / per-row X; spend chrome in Edit mode |
| **TASK-587** | DEV-V-009 **T035** | Defense Score value tip (`defenseScoreHelp`); name tips unchanged; roll chips still work |
| **TASK-586** | DEV-V-009 **T033–T034** | Temp Modifier on header LargeStatBlocks (Speed/Evasion/DR/crit) + Health `Terminal: X` + Abilities (+ defenses + resource-maxima toggle) / Skills in **Temp mode**; tint + persist + cascade; Edit spend locks (Speed/Evasion pencil removed — see TASK-600 / TASK-782) |
...
| **TASK-512** | DEV-V-008 **T015** (+ DEV-V-009 **T053**) | Sheet header DR + Critical Range only when armor modifies that stat; crit = Evasion + 10 + (1 + op_1_lvl) |
...
| **DEV-Q04** | TASK-780 feat rank UX | **Resolved:** Option A. Collapsed Lvl stepper removed; edit rank via expanded Feat Levels chips. Creature creator stepper left as a later follow-up. |
| **DEV-Q05** | Armament table split | **Ready for owner review (TASK-871 partial).** Recommendation: **keep** `official_items`/`user_items` + `type` (live official 17/5/2, user 29/9/6). Split = 6 tables + RLS/API/enrichment blast. Real fix is TASK-872/873. **Do not migrate** until you pick keep vs split + ADR. |
| **DEV-Q06** | Feat category Offense → Offensive | **Ready to apply (TASK-876 partial).** Live: 8 `Offense` / 187 `Offensive`. Seed already 0/183. Filters show Offensive once (`normalizeFeatCategory`). SQL preview in `sql/codex-feat-category-offense-to-offensive-proposed.sql`. Say **apply** for live UPDATE. |
| **DEV-Q07** | Admin changelog retention | **Display + GET slimming shipped (TASK-874 partial).** UI + API ship field diffs / name-only (not full dumps). Writes still store snapshots. Live: 709 rows / ~1.7 MB. Proposal: 90-day TTL; drop snapshot columns only if no restore path. Ack before schema/TTL apply. |
| **DEV-Q08** | Auth CAPTCHA + signup abuse (TASK-899) | **Open — owner Dashboard + keys.** Frontend Turnstile wiring is in repo; protection is inactive until Cloudflare site key on Vercel **and** Supabase Bot and Abuse Protection enabled with matching secret. Also tighten Auth rate limits + password min while in Dashboard. `security.txt` ships with deploy — verify `/.well-known/security.txt` on production. |
