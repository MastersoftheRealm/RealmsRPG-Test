...
| **Steps** | 1. Open Add Power at ~360px. 2. Confirm the modal shows a compact row with **Search** and a **Filters** button (not a stack of SourceFilter / Powers–Empowered tabs / forms above the list). 3. Confirm a one-line options summary (e.g. source · Powers) under the toolbar when Filters are closed. 4. Confirm several list rows are visible between the toolbar and the sticky footer without opening Filters. 5. Tap **Filters** — source / mode (and equipment custom-add when relevant) appear in the panel; Hide Filters collapses them again. 6. Open Add Feat — Filters starts collapsed; opening it reveals category/ability/checkboxes; list remains the main scroll region. 7. Spot-check Crafting “Select item to craft”, Advanced Powers add, and a creator Load modal: same list-first chrome + summary when Filters closed. 8. Spot-check desktop: Filters not permanently expanded. |
| **Expected** | `headerExtra` + `filterContent` live in collapsed-by-default compact `FilterSection`; list is the dominant focus; `optionsSummary` / badge when options are non-default; sticky footer still works (T013). |
