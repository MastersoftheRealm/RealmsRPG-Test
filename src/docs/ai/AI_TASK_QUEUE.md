...
`npm run tasks:validate` — strict reconcile, doc links, FEATURE_INDEX + generated barrels, related_files, shared/ui allowlist. Prefer `TASK-###` in commit messages. Uploads: eslint `realms/no-raw-upload-fetch`.
