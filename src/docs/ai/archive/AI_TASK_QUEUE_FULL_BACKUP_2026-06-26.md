...
> **Queue hygiene (2026-06):** Many completed/cancelled tasks were moved to
> [`archive/TASK_QUEUE_DONE.md`](archive/TASK_QUEUE_DONE.md), but this file still contains historical
> `done` items retained for context and audit traceability.
>
> **Status update (2026-06-26):** The prior merge-warning note for TASK-305/306/307/308/310 is
> obsolete. These tasks are already validated as `done` in current remediation status.
...
- id: TASK-378
  ...
  created_by: agent
  description: |
    Finish HYG-01 safely by replacing `fetchCodex`'s `any`-based response typing with a canonical
    typed payload, while preserving compatibility adapters for historical data shapes currently
    still tolerated in roll/campaign displays.
  related_files:
    - src/lib/api-client.ts
    - src/hooks/use-codex.ts
  ...
  acceptance_criteria:
    ...
    - `photoURL` semantics are documented as active auth/profile alias (not blanket-removed as dead Firebase shape).
    - `npm run build`, `npm test`, and `npm run lint` pass.
    - Build validation suite added/indexed for codex + roll-log compatibility checks.
  notes: |
    Planned from remediation close-out review (2026-06-26). Must be done in compatibility-first
    phases to avoid breaking codex selectors and historical roll payload rendering.
...
- id: TASK-379
  ...
  created_by: agent
  description: |
    Complete deferred duplication cleanup by unifying add/load library selection pipelines into one
    builder+normalizer path, then refactor `LoadFromLibraryModal` into a thin wrapper over
    `UnifiedSelectionModal` with consistent list/search/sort/filter behavior.
  related_files:
    ...
    - src/docs/ai/BUILD_VALIDATION.md
  acceptance_criteria:
    - Single shared pipeline for selectable-item shaping is used by add + load flows.
  ...
  notes: |
    Deferred from remediation waves due shared-flow regression risk. Requires QA-first execution.
...
- id: TASK-380
  ...
  created_by: agent
  description: |
    Introduce shared creator-page shell scaffolding and consolidate remaining collapsible patterns
    only after parity tests exist for each creator route.
  related_files:
    ...
    - src/docs/ai/BUILD_VALIDATION.md
  acceptance_criteria:
    - Shared CreatorPageShell (or equivalent) removes duplicated auth/load/save scaffolding.
    - Collapsible usage is consolidated to current supported patterns (no dead references).
    - Creator page behavior remains equivalent across all six routes.
    - `npm run build`, `npm test`, and `npm run lint` pass.
    - Dedicated creator parity validation suite added before merge.
  notes: |
    This task is sequencing-dependent: execute after TASK-379 and associated QA harness.
...
- id: TASK-381
  ...
  created_by: agent
  description: |
    Decompose large character-sheet/creator files via phased extractions (domain hooks and server
    shell + client islands) only with test-backed parity checkpoints and rollback-safe PR slicing.
  related_files:
    - src/components/character-sheet/use-character-sheet-actions.ts
    - src/app/(main)/characters/[id]/page.tsx
    ...
    - src/docs/ai/BUILD_VALIDATION.md
  acceptance_criteria:
    - `use-character-sheet-actions` is split by domain boundaries without behavior regressions.
    - Targeted large creator/sheet routes are decomposed into stable shells/islands in phases.
    - Each phase ships with explicit parity validation and rollback plan.
    - `npm run build`, `npm test`, and `npm run lint` pass per phase.
  notes: |
    Deferred from remediation waves due high blast radius on critical untested flows. This task
    should proceed only with expanded DEV-V validation coverage and small-scope PRs.
...
- id: TASK-382
  ...
  created_by: agent
  description: |
    Keep active docs lean by moving non-actionable historical task blocks out of `AI_TASK_QUEUE.md`
    and pruning stale references in active guidance docs so agents/devs do not chase dead routes or
    superseded audit states.
  related_files:
    ...
    - src/docs/README.md
  acceptance_criteria:
    - `AI_TASK_QUEUE.md` contains only active/pending work; done/cancelled historical blocks moved to archive.
    - Active docs contain no stale references to removed routes/components unless explicitly marked historical.
    - Archive pointers are explicit so history remains recoverable.
    - `npm run build` passes.
  notes: |
    Created from post-remediation docs audit to keep agent context focused and reduce accidental
    rework from stale references.
