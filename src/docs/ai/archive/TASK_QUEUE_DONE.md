...
Firestore/RTDB � the project is Supabase-only.

---

- id: TASK-486
  title: Weapons property density — one bullet per line under name
  ...
  description: |
    ...
    render one `• Property` per line under the weapon name (layout A). Apply globally via
  ...
  acceptance_criteria:
    - Weapon properties render one `• Property` per line under the name in QuickWeaponsTable.
  ...
  evidence: |
    ...
    Before: displayNamedProperties joined as inline `• a • b • c` under name — wraps/crams at ~360px when multiple named props.
    After: NamedPropertiesUnderName stacks one `• Name` div per property under the name cell
    (text-text-muted / dark:text-text-secondary; index keys; not a semantic list — visual bullets only).
    ...
    Character consumer: archetype-section WeaponsSection → QuickWeaponsTable (Unarmed via trailingRows in same tbody — columns share widths; property stack does not touch Unarmed cells).
    Creature audit: creature-stat-block uses WeaponsListSection (GridListRow + property chips), not QuickWeaponsTable — no parallel cramped-join path; shared-table AC covers consumers of QuickWeaponsTable (character today).
...
- id: TASK-485
  ...
  description: |
    On character sheet normal (non-edit) view: sub-skills keep └ + italic but match base skill
  ...
  evidence: |
    ...
    decorative └ aria-hidden; tasks:validate.
  notes: |
    Completed 2026-07-16. No SkillRow fork — play chrome gated on existing isEditing.
...
- id: TASK-274
  ...
  related_files:
    ...
    - src/app/(main)/creature-creator/creature-creator-constants.ts
  acceptance_criteria:
    - Damage Modifiers section: show feat point cost per type (resistance, immunity, weakness) � e.g. label or chip like "+1 pt" / "-1 pt" � before adding (in dropdown or nearby) and on each added chip/row.
...
## Appended 2026-07-15 (workflow slim � 79 tasks from active queue)
...
- id: TASK-474
  title: AI agent workflow overhaul � constitution, ACTIVE_TASKS, CI, Library shell
  created_at: 2026-07-15
  created_by: agent
  priority: high
  status: done
...
- id: TASK-482
  ...
  notes: |
    Shipped in audit follow-up pass 2026-07-15. Remaining debt: TASK-476�481.
