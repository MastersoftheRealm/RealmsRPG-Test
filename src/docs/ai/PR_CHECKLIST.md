...
15. **Human gates** — New shared/ui file on allowlist + ADR? Store / API contract reviewed?
16. **Uploads** — No raw `fetch('/api/upload…')` (eslint `realms/no-raw-upload-fetch`)?
17. **Barrels** — If you changed shared/ui/hooks/services exports, ran `npm run tasks:generate-index`?
