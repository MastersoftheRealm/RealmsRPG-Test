...
- **CI:** `.github/workflows/ui-verify.yml` runs contrast, lint, build, visual + a11y baselines, and the **shell-creators chrome audit** (`playwright.shell-creators-audit.config.ts` — 8 tests, ~1–2 min on Linux after build). Other audit configs (`verify:creator-audit`, loadout, flaw, feat-cards, chip-unification, …) remain manual/optional via dedicated `playwright.*.config.ts` scripts.
