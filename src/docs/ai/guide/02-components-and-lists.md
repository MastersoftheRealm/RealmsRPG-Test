...
**List modal layout (add-X, load, selection):** Prefer **`UnifiedSelectionModal`** (or thin wrappers: `AddLibraryItemModal`, `LoadFromLibraryModal`, `AddFeatModal`, `AddSkillModal`, `AddSubSkillModal`, `AddCreatureFeatModal`, `AddProficiencyModal`, `CraftingItemSelectModal`, guided L2 feats/equipment/powers) so search/sort/list/footer stay consistent. Remaining gated parallel shell: AdminSpecies trait picker (TASK-572). `UnifiedSelectionModal` defaults to **`flexLayout`** + **`fullScreenOnMobile`** and passes Cancel / Add Selected through Modal’s **`footer`** slot so actions stay pinned on phones (do not put confirm buttons inside scrollable children). For rare custom lists that cannot use UnifiedSelectionModal, use **useModalListState** (`@/hooks/use-modal-list-state`) for search/sort state — and apply `gridColumnsWithInlineSelection` yourself (do not pre-wrap grids passed into UnifiedSelectionModal). Structure: (1) Modal title/description header, (2) **compact toolbar** — Search + optional **Filters** toggle on one row, (3) **`scopeExtra`** — always-visible primary mode tabs (Powers/Empowered, Armaments/Equipment, etc.), (4) collapsible options panel (`headerExtra` + `filterContent` via `FilterSection` `variant="compact"`, collapsed on open — SourceFilter, advanced filters, custom-add), (5) **ListHeader**, (6) scrollable **GridListRow** list (primary focus), (7) Modal **`footer`**. Put catalog-identity tabs in **`scopeExtra`**; put SourceFilter / custom-add / advanced filters in **`headerExtra`** (TASK-564). Optional `optionsSummary` / `optionsActiveCount` surface secondary options when collapsed. Use **EmptyState** / **LoadingState**; avoid ad-hoc Spinner/divs. **Quantity (`showQuantity`):** in-row **`QuantitySelector`** (quantity-first: raise qty from 0 to select, lower to 0 to deselect) — do not place a side-column stepper that shoves the row.

**Encounter / session participant picker (intentional non-USM — TASK-571):** Use **`AddCombatantModal`** (`@/components/shared`) for adding combatants/participants to combat, skill, mixed, downtime, VTT, and similar session-play surfaces. Same class of alternate as **`RealmsImagePicker`** (domain shell, not a catalog USM wrapper): library ↔ campaign scope, campaign character drill-down, quantity + enemy/ally/companion type, and initiative / encounter payload on confirm. **Extend this component** for new session-play pickers — do not fork a parallel modal and do not migrate this grammar onto UnifiedSelectionModal.
...
| UnifiedSelectionModal | AddFeat/Skill/SubSkill/LibraryItem/Proficiency, LoadFromLibrary, CraftingItemSelect, AddCreatureFeat, guided L2 (feats/equipment/powers) |
| AddCombatantModal | Encounter / session participant picker (combat + skill today; VTT / downtime / future — extend, don’t fork; TASK-571) |
| library-selectable-builders | Add + Load library SelectableItem shaping (shared pipeline) |
| useModalListState | Rare non-USM list shells only (AdminSpecies trait picker until TASK-572). Not for AddCombatantModal — that is its own shared shell. |

**Intentional exceptions:** Auth pages use `gray-*` / brand social colors; AddSubSkillModal uses SelectionToggle (not GridListRow); **AddCombatantModal** is the shared encounter/session participant picker (non-USM; TASK-571); filled primary/danger controls use `text-text-on-dark` on colored backgrounds.
