...
**Intentional exceptions:** Auth pages use `gray-*` / brand social colors; AddSubSkillModal uses SelectionToggle (not GridListRow); MixedSpeciesModal is a dual `<select>` pair (not USM / GridListRow — TASK-605); filled primary/danger controls use `text-text-on-dark` on colored backgrounds.
