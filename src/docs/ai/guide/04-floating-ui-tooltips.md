...
| Definition tip on a label word (ability/defense/skill name or Score value; no icon) | **`WordHelpTip`** + `getAbilityHelp` / `getDefenseHelp` / `defenseScoreHelp` in `tooltip-text.tsx` (name tips say the name once — e.g. “Acuity reflects…”, not “Acuity. Acuity…”). Skill names pass Codex `description` and `compact` (TASK-802). | Info icon sibling, `title=` only |
