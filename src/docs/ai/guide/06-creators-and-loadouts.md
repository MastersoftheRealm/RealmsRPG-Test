...
- **Collapsibles:** Use **`CollapsibleSection`** only (`ui/Collapsible` removed). Sections and **`FilterSection`** panels default **collapsed** on first open (summary line visible when provided). Expand control is a dedicated `<button>`; `rightSlot`/Remove sit outside it; section titles are `h2` (under page `h1`). Chrome screenshot audit: `npm run verify:shell-creators-audit` → `.shell-creators-audit/` (also runs in CI via `ui-verify.yml`).
