...
- **Task:** TASK-806 (catalog + CI), TASK-807 (resolver drives list chrome), TASK-814 (demote overflow to chips)
...
4. **Surface bindings** — CI pointers `{ entityType, mode, flags }`. New lists register a binding, not a custom fact table.
...
- **Positive:** One SoT for “what must be visible”; Official / guided / USM / sheet chrome can share a resolved layout; gear cannot grow combat columns by accident. Mixed weapon+armor+shield USM lists are retired (**TASK-810**): Armament Load and creature Inventory pick per kind (sheet Add was already split). Codex/Admin equipment stays the closed gear set.
