...
4. **Surface bindings** — CI pointers `{ entityType, mode, flags }`. New lists register a binding, not a custom fact table. Path More details power/technique catalogs (`DetailOptionList`) bind `detail-option-power` / `detail-option-technique` at detail density (column budget 0; name collapsed, description expanded; all valued facts are chips).
