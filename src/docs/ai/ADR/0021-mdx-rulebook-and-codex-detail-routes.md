...
1. **Rulebook is in-repo MDX** under `src/content/rules/`, compiled with `@next/mdx` + `remark-gfm`.
   `/rules` is a chapter index; `/rules/[slug]` renders one chapter as an RSC with
   `generateStaticParams` + `generateMetadata` and in-page chapter/section nav. Keep the Google Doc
   as a view-source link. Do **not** store the manuscript in `core_rules` (that table is formula
   overrides, not prose).
