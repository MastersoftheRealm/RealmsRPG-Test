...
- **C1 Side-scroll.** A horizontal panel carousel must bound the play surface so off-screen siblings cannot lengthen the page; must not stretch panels to the tallest sibling; must show a visible affordance that more content exists horizontally without greying out the active panel; and must let keyboard and non-touch users reach off-screen items without a swipe gesture.
