...
- **Negative / follow-ups:** the tier migration is not free. `Button`, `IconButton`, `ValueStepper`, `Input`, `Select`, `Textarea`, `.tab-nav-trigger`, `.tab-pill-trigger`, and `.touch-target-md-compact` all need updating (TASK-830, TASK-836, TASK-841). Until then the baseline holds the line rather than improving it. The gate adds ~2–3 min to `ui-verify`.
