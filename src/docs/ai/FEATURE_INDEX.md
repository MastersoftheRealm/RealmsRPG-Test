...
| Advanced equipment catalog / budget (TASK-596) | `lib/creator/advanced-equipment-catalog.ts` (+test) — merge user/codex/public rows, path recommend resolve, currency/TP/unarmed helpers, inventory add/remove/replace; wired by `character-creator/steps/equipment-step.tsx` (inline list + `CreatorResourceBar`; reuses guided `equipment-currency` starting/spend math). Not LoadoutBudgetBar / USM. |
