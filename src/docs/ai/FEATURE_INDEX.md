...
| Guided powers/techniques L1 + L2 (TASK-444 / TASK-456 / TASK-458 / TASK-461 / TASK-463 / TASK-470–472 / TASK-573) | `steps/powers-techniques-step.tsx`, `guided-powers-techniques-l2-modal.tsx`, `powers-techniques-l1-candidates.ts`, `powers-techniques-l2.ts`, `powers-techniques-energy-filter.ts`, `power-technique-display.ts`, `loadout-tp.ts`, `LoadoutBudgetBar`; L1 path cards (innate vs regular for Power); Training Points shared with Loadout for **innate + regular** picks; Innate Energy PointStatus + threshold gate (under-fill = soft warn, Continue stays on); innate cards same TP title chip as regular (Energy in detail); L2 `UnifiedSelectionModal` (Energy ≤ theoretical L1 max / innate ≤ threshold + shared TP); L2→L1 promotion; Action Type value-only desc chips. |
| Advanced character creator (classic 9-step) | `characters/new/advanced/page.tsx`, `stores/character-creator-store.ts` (`getCharacter` sets `libraryTabVisibility` via `defaultLibraryTabVisibilityForArchetype`); step facades in `character-creator/steps/*-step.tsx` (barrel `steps/index.ts`). TASK-598: `finalize-step` → `steps/finalize/*`; `feats-step` → `steps/feats/*`; `equipment-step` → `steps/equipment/*`; `powers-step` → `steps/powers/*` + `lib/creator/advanced-powers-selectable.ts` (named exports unchanged). |
| Advanced equipment catalog / budget (TASK-596) | `lib/creator/advanced-equipment-catalog.ts` (+test) — merge user/codex/public rows, path recommend resolve, TP/unarmed helpers, inventory add/remove/replace; wired by `character-creator/steps/equipment-step.tsx` facade + `steps/equipment/` UI panels (list-columns, unarmed-prowess-panel, path-loadout-section, selected-equipment-list, equipment-catalog-panel, step-header). Starting/spend currency from guided `equipment-currency` (imported by step). Not LoadoutBudgetBar / USM. |
...
| Power creator | `power-creator/page.tsx` (shell + bootstrap) + `use-power-creator-workspace.ts` + `power-creator-editor.tsx` (TASK-381); guided: `power-creator/guided/page.tsx` (TASK-410+) — chrome: `CreatorPageShell` |
...
| Guided ancestry pick task order | `buildAncestryPickTasks` (`lib/guided-creator/ancestry-pick-tasks.ts`) — characteristic → ancestry trait → optional flaw → bonus ancestry if flaw taken; used by `ancestry-step` (TASK-591; vitest DEV-V-013-T061) |
