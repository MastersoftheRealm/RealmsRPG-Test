...
| Selection modal (add/pick from library) | `UnifiedSelectionModal` (default `flexLayout` + `fullScreenOnMobile`; list-first chrome: Search + Filters toolbar, `headerExtra`/`filterContent` collapsed in `FilterSection` compact; Cancel/Add Selected via Modal `footer`; `maxSelections` soft limit when max ≠ 1; `maxSelections={1}` replaces selection; `confirmLabel` / optional `primaryActions`) |
