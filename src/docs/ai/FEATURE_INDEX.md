...
| Part/property chips | `ExpandableChip` + `expandableChipPropsFromPartData` (`lib/chip/expandable-chip-props.ts`); `PartData` in `lib/chip/part-data.ts` (re-exported from `@/components/shared`); `partChipsFromDisplay` in `lib/chip/part-chips-from-display.ts` — `PartChip` / `PartChipComponent` deleted (TASK-569) |
