...
| Ability score grid | `AbilityScoreGrid` (`ability-score-grid.tsx`): display/edit tiles; `powerAbility` / `martialAbility` / `secondaryAbility` pills; `resolveDistinctSecondaryAbility`; mobile display uses `shortName` below `sm`; path pills use short single-line copy (Archetype / Secondary) with full aria-label + tile top padding so wrap cannot cover the name (TASK-452, TASK-455); roomier edit grid; ability names use `WordHelpTip` + `getAbilityHelp` (TASK-544) |
