...
Last updated: 2026-08-21 (Codex/library GLR, terminology, changelogs, guided creator, sheet, mobile systemic)
...
- Misinterpretation / code note: Category chips use `labeledFactChip('Category', …)` (`list-row-metadata.ts`); Action Type is already value-only. Armament Currency chips: TASK-870 → `resolveItemMarketPricing`. Armaments live in `official_items`/`user_items` with `type`; Equipment is already `codex_equipment`. GLR entity type is still internal `'gear'` (= Equipment); play density `columnBudget.gear = 0` and `playEquipmentChrome` extraColumns Type+Qty, so sheet Equipment is still Name/Type/Qty until TASK-873. Changelogs persist full `before_data`/`after_data`. USM footer is `border-t` with `footerExtra` flush. Seed `feats.csv` has both `Offense` and `Offensive`. Path filter previously used `xl:col-start-4` (fixed TASK-877 — last sibling / `min-w-0` only). Innate eligibility covers Basic + healing-part bans, not duration ≤ 1 minute or Adaptation-category parts. Age is `mergeAgeIntoAppearance`. Toolbar red dot is `hasUnappliedPoints` (unspent slots / level-up), not section-pencil overspend. Name cells use `break-words`.
- Disposition: Filed **TASK-869–898** (ACTIVE). **(8) TASK-877 done** — Path filter last-in-flow, no `col-start` lock. **(4) TASK-872 done** (pending-qa) — player Gear gone; Equipment / Armaments / Enhanced vocabulary; sheet Equipment columns remain **TASK-873**. Architect research **TASK-871** (no schema split until owner ack). Codex data **TASK-876** (Offense→Offensive: audit → propose SQL → owner apply). Cross-ref: no open ACTIVE dupes; WAITING TASK-823/834 unrelated. ADR-0014 path filter (TASK-751–753); **TASK-878** still open (Any / Any Power / …). Innate SoT **TASK-879** (`innate-eligibility.ts` + GAME_RULES). Mobile wrap/overlap **TASK-897/898** (MOBILE_UX C2 / GLR expanded).
