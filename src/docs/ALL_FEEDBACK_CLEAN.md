...
- Priority: High (security / abuse)
- Feedback (verbatim summary): Signup posts to Supabase `/auth/v1/signup` with public key; no CAPTCHA enforced (`gotrue_meta_security.captcha_token` never required); rate limiting not observed on direct API calls; bulk scripted signups + confirmation-email spam possible. Fix: enable Supabase Attack Protection CAPTCHA (Turnstile/hCaptcha), wire widget on signup form, tighten Auth rate limits + password policy. Missing `/.well-known/security.txt` (RFC 9116); suggest non-email contact channel.
- Misinterpretation / code note: Publishable key in bundle is expected. Gap is missing Dashboard CAPTCHA + frontend token + optional rate-limit tuning. Resend confirmation is server rate-limited; signup bypasses Next.js entirely. Supabase MCP cannot toggle CAPTCHA.
- Disposition: **TASK-899 partial** — Turnstile widget on register/login/forgot-password when `NEXT_PUBLIC_TURNSTILE_SITE_KEY` set; `security.txt` added; DEPLOYMENT rollout doc. Owner **DEV-Q08**: Turnstile keys + Supabase Dashboard CAPTCHA + rate limits + HIBP (DEV-001).
...
- Disposition: Implemented directly — VTT campaign access enriches roster portraits from live character rows before token seeding/backfill; fallback roster portrait behavior preserved. Covered by `src/lib/tabletop/tokens.test.ts`; manual QA DEV-V-056-T011.

**Raw Feedback Log — 2026-08-21 (Codex/Library + maintenance + guided creator + sheet + mobile)**
- Context: Codex/Library GLR; admin changelogs; add-skill/sub-skill modals; feats data; archetype-path filters; innate-power filters; guided Abilities/Skills; character sheet header/library/rolls; mobile thin screens
- Priority: High (wrong facts, mixed game terms, play-surface bugs, plus Architect research)
- Feedback (verbatim summary): (1) Power/technique expanded desc chips say “Category Defense” instead of “Defense”. (2) Weapon/armor/shield desc chips show currency multiplier, not library/creator cost. (3) Owner concern: split armaments (weapons/armor/shields) out of official/user items into separate tables. (4) “Gear” is not a game term; Equipment vs Armaments vs Enhanced Items get mixed; sheet Inventory Equipment GLR shows Name/Type/Qty and lacks Description/Category/Currency/Rarity. (5) Admin changelogs store/display too much (full item dumps). (6) Add Skills / Add Sub-Skills “Browse all …” sits flush on the footer rule. (7) Feats categories duplicate Offense vs Offensive — unify to Offensive. (8) Archetype Path filter is CSS-locked bottom-right instead of last-in-flow. (9) Add Any / Any Power / Any Martial / Any Powered-Martial path options (also guided L2 feats). (10) Innate power = BASIC action/reaction, duration ≤ 1 minute, no Adaptation parts. (11) Guided Abilities cards too large vs sheet; allocation needs a player tooltip. (12) Guided Skills defense cards should match sheet (score+bonus, steppers hugging value, centered +1 (2sp)). (13) Add Skills modal: abilities as library columns not chips/abbreviations/duplicates; column too far right. (14) Add Sub-Skill modal: abilities full names, not abbreviated. (15) Mixed-species header wraps as “Level Species1 /” then “20 · Species2”; wrap Level vs species. (16) Speed/Evasion/Crit/DR space-fill the header. (17) Creator age lands in appearance notes; age should be a Notes header field; backstory missing. (18) Edit-mode hide-eye padding crowds library tabs. (19) Feats Customize looks off vs desc chips; expanded divider touches chips. (20) Feat names should be “Name II” not “Name (Level 2)”. (21) Library tab scroll arrows show when tabs already fit. (22) Prof-points pool ± should go; slider thumb off-center / no dark variant. (23) Level-up auto-assign a new prof point to the existing martial-only or power-only side. (24) Roll-log bonus hover should name the bonus (Strength, Power bonus, Reflexes bonus). (25) HP may go negative. (26) Click/tap outside the roll log closes it unless starting another roll. (27) Red edit-pen dot with no local red pencils. (28) Thin screens: character-by-character wrap (weapon names); expanded GLR overlap (feat Uses steppers vs Recovery) — treat as global.
- Misinterpretation / code note: Power/technique Category desc chips use `categoryFactChip` (value-only; TASK-869); feat/gear still `labeledFactChip('Category', …)`. Action Type is value-only. Armament Currency chips: TASK-870 → `resolveItemMarketPricing`. Armaments live in `official_items`/`user_items` with `type`; Equipment is already `codex_equipment`. GLR entity type is still internal `'gear'` (= Equipment); sheet play surface `character-sheet-gear` uses `layoutOverrides` for Category/Currency/Rarity columns + Description/Qty extras (TASK-873 done, pending-qa). Creature selected equipment uses `creature-inventory-equipment` (Type/Qty columns + chip-all facts). Changelogs persist full `before_data`/`after_data`. USM footer `footerExtra` uses shared `pt-3 md:pt-4` below `border-t` (TASK-875 done, pending-qa). Seed `feats.csv` has both `Offense` and `Offensive`. Path filter previously used `xl:col-start-4` (fixed TASK-877 — last sibling / `min-w-0` only). **(10) Innate eligibility (TASK-879 done, pending-qa):** `isPowerInnateEligible` in `innate-eligibility.ts` + GAME_RULES — Basic/Basic Reaction, duration ≤1 minute (unknown fails closed), no Adaptation-category parts, plus healing/energy-gain bans; wired through library filters, guided innate L2/L3, admin path validation. Age/backstory use dedicated JSON fields (`resolveCharacterAge`; save strips legacy `Age: N` prefix via `normalizeAgeAppearanceForSave` — TASK-886). **(27) Edit-pencil dot (TASK-896 done, pending-qa):** `sheet-edit-notification` — green unspent, red overspent, red/green gradient when both, purple level-up when balanced, tooltip lists pools; section pencils unchanged (red overspend / green unspent). Name cells use `break-words`.
- Disposition: Filed **TASK-869–898** (ACTIVE). **(1) TASK-869 done** (pending-qa DEV-V-016 T032) — power/technique Category desc chips value-only via `categoryFactChip`; feat/gear keep labeled Category chips. **(4) TASK-872 done** (pending-qa) — player Gear gone; Equipment / Armaments / Enhanced vocabulary. **(4) TASK-873 done** (pending-qa DEV-V-016 T031) — sheet Inventory Equipment GLR: Name, Description, Category, Currency, Rarity, Qty; Add-equipment persists catalog currency/category/rarity. **(6) TASK-875 done** (pending-qa DEV-V-013 T010) — shared USM footer padding below separator for Browse-all layer nav. **(8) TASK-877 done** — Path filter last-in-flow, no `col-start` lock. **(9) TASK-878 done** (pending-qa) — Quick select aliases (Any / Any Power / Any Martial / Any Powered-Martial); guided auto-select uses one type alias. **(10) TASK-879 done** (pending-qa DEV-V-046 T009) — unified innate eligibility (duration + Adaptation). **(27) TASK-896 done** (pending-qa DEV-V-009 T067) — toolbar edit dot matches section pencil grammar (green/red/mixed/purple level-up). **(28) TASK-898 done** (pending-qa DEV-V-016 T033) — expanded GLR column facts reflow in body; Uses steppers no longer overlap Recovery in narrow panels. Architect research **TASK-871** (no schema split until owner ack). Codex data **TASK-876** (Offense→Offensive: audit → propose SQL → owner apply). Cross-ref: no open ACTIVE dupes; WAITING TASK-823/834 unrelated. ADR-0014 path filter (TASK-751–753 / TASK-878). Mobile wrap **TASK-897** (MOBILE_UX C2 / truncate).

**Raw Feedback Log — 2026-08-21 (Mobile character sheet: header, swipe, GLR expand)**
- Context: Character sheet below `md`; Library → Feats GLR expand
- Priority: High (play-surface layout + duplicated facts)
- Feedback (verbatim summary): (1) Scrolling down on mobile, the header section does not fully disappear and takes up half of the lower character sheet. (2) Swiping the lower section, it is not centered below the header. (3) GLR expanded mobile: when feats are expanded, Uses and Recovery (and other column details) appear in both the header and the expanded body; they should be in the header when collapsed, in the body when expanded, never both. Empty values (Uses `-`) should not display anywhere.
- Misinterpretation / code note: After TASK-838 the column kept a `flex-1` carousel with a `min(50dvh)` floor, so the sheet header could only partially scroll away. Nested panel scroll never collapsed it. Carousel `-mx-4` + `px-4` + `scroll-px-4` + `w-full` desynced snap from the header. GridListRow already hid description teasers when expanded but still painted mobile summary + expanded body stats (including `-` placeholders) until TASK-868/898.
- Disposition: **TASK-868** — collapse `data-sheet-mobile-header` on panel scroll (header is not an inner scroller); C1 rest fade via `tabListOverflowState`; snap panels to PageContainer/header width (no scroller padding); mobile column facts XOR expanded body; omit blank/`-` via `columnHasDisplayValue`. DEV-V-009 T064 + T021, DEV-V-016 T029.
...
- Priority: High
- Feedback: Make the public library accessible on the Library page instead of the Codex by switching between "public" vs "my library"; update page title by which library you're on. Remove public library from Codex. Powers, techniques, armaments, and creatures in public tabs on the Library page. Add a public library edit mode to the admin page that uses the same format and editing methods as the Codex editor but for saved/published public library items (techniques, powers, armaments, etc.). Use common sense for tabs/pages migrations and remove old/useless functionality.
