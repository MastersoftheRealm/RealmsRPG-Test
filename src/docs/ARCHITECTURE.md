...
2. **Character sheet:** Uses `useCharacter` (shared `characterKeys.detail` cache — TASK-750), `useUserPowers` / `useUserItems` / …, Codex hooks. Enrichment happens in `library-section.tsx` and similar components. Campaign RM view uses `useCampaignCharacterView` / `campaignKeys.characterView` against the campaign route (TASK-761) — not `characterKeys.detail`, which does not enforce roster + RM auth. Combat linked-character HP sync uses `useCampaignCharacterEncounters` / `campaignKeys.characterEncounter` → `getCampaignCharacterForEncounter` (`?scope=encounter`; TASK-762) — not the RM-view GET.
