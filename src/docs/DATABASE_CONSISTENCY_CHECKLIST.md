...
| **Character sheet** | Owner: `enrichCharacterData` from catalog hooks (`useUser*` / `useOfficialLibrary` / `useCodex*`). Other-user + RM view: `libraryForView` + referenced `enrichment` on the GET (TASK-773); `sheetCatalogFromEnrichment` maps that onto the same derived args. | Powers/techniques/items must be resolvable by id (or name fallback) in owner library rows or official rows in the payload. Do not fetch the *viewer's* library for a read-only sheet. |
...
| **Enrichment** | `enrichPowers`, `enrichTechniques`, `enrichItems` take optional `publicPowerLibrary` etc. for character-referenced official items. Read-only GET catalog: `getCharacterViewEnrichment` (server) + `sheetCatalogFromEnrichment` (client). | Owner sheet still builds `publicLibraries` from `useOfficialLibrary`. RM / other-user sheets use `enrichment.official*` from the character GET. |
