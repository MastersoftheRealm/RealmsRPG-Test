...
- **Implementation:** Prefer responsive sizing: `touch-target-md-compact` (utility in `globals.css`), `min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0`, or `--touch-target-min: 44px` on steppers/toggles. Use `@media(pointer:coarse)` on `Button`/`IconButton` where the control stays compact on desktop only. For icon-only help (`InfoTippy`) beside labels/headings, use `.hit-area-layout-neutral` so the 44px (mobile) hit overlay does not stretch the label row.
...
- **Sticky action buttons:** Put primary actions (**Add Selected**, Confirm, Load, Pick Me, etc.) in the Modal **`footer`** prop — never inside `children`. The footer is `shrink-0` outside the scroll region so users do not scroll to reach it on phones. Modal applies footer inset (`px-4 py-3` / `md:px-6 md:py-4`) — do not add a second `p-4`/`px-6` on the footer row. `UnifiedSelectionModal` already wires Cancel / Add Selected (or `confirmLabel` / `primaryActions`) through `footer`. Avoid bottom padding on the scroll content that creates a blank strip above the sticky footer (`pb-0` on USM content — TASK-574).
- **List-first selection chrome (TASK-564 / TASK-815):** In add/load/`UnifiedSelectionModal` flows, the **scrollable list is the primary focus**. Search and a **Filters** toggle share one compact toolbar row. **Primary mode tabs** (Powers/Empowered, Armaments/Equipment, feat-source, inventory type) and the sheet **Add equipment custom-item form** stay **always visible** via `scopeExtra` under search. SourceFilter and advanced filters live in a **collapsed-by-default** `FilterSection` (`variant="compact"`) as `headerExtra` / `filterContent`. Optional `optionsSummary` / `optionsActiveCount` show secondary options when Filters are closed.
