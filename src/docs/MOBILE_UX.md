...
- **List-first selection chrome (TASK-564):** In add/load/`UnifiedSelectionModal` flows, the **scrollable list is the primary focus**. Search and a **Filters** toggle share one compact toolbar row; source tabs, mode switches, custom-add forms, and advanced filters live in a **collapsed-by-default** `FilterSection` (`variant="compact"`). Do not stack always-visible filter rows between search and the list — they squeeze the list against the sticky footer on phones and short desktop viewports. Optional `optionsSummary` / `optionsActiveCount` show what’s applied when Filters are closed.
