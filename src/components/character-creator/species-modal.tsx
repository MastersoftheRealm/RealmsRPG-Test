...
function SpeciesCatalogTraitSection({ title, traits }: SpeciesCatalogTraitSectionProps) {
  ...
  return (
    <div className="mb-5 space-y-4">
      <div className="font-nunito text-sm font-semibold tracking-wide text-text-secondary uppercase">
        {title}
      </div>
      {plainItems.length > 0 ? (
...
export function SpeciesModal({ species, traits, isOpen, onSelect, onClose }: SpeciesModalProps) {
  ...
  const sizesDisplay =
    Array.isArray(species.sizes) && species.sizes.length > 0
      ? species.sizes.join(' / ')
      : species.size || 'Medium';
  ...
  return (
    ...
    >
      <div className="p-6">
        ...
        <Card className="mb-6 grid grid-cols-2 gap-3 bg-surface-alt p-4 shadow-none md:grid-cols-4">
          <div className="text-center">
            <span className="block text-xs tracking-wide text-text-muted uppercase">Size</span>
            <span className="font-bold text-text-primary capitalize">{sizesDisplay}</span>
          </div>
          <div className="text-center">
            <span className="block text-xs tracking-wide text-text-muted uppercase">Type</span>
        ...
        {species.ability_bonuses && Object.keys(species.ability_bonuses).length > 0 && (
          <div className="mb-6">
            <h4 className="mb-2 text-sm font-semibold tracking-wide text-text-secondary uppercase">
              Ability Bonuses
            </h4>
        ...
        {resolvedTraits && (
          <>
            ...
            />
            <SpeciesCatalogTraitSection title="Flaws" traits={resolvedTraits.flaws} />
          </>
        )}
      </div>
    </Modal>
