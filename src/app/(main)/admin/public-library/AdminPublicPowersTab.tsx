 ...
 * List + edit modal, same pattern as Codex editor.
 */
...
export function AdminPublicPowersTab() {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/public/powers?id=${encodeURIComponent(editing.id)}`, { method: 'DELETE' });
    ...
    <div>
      ...
      <ListHeader
        columns={[
          ...
          { key: 'actionType', label: 'ACTION', sortable: false as const },
      ...
      <div className="flex flex-col gap-1 mt-2">
        {isLoading ? (
          ...
          <ListEmptyState icon={<Wand2 className="w-8 h-8" />} title="No public powers" message="Add one from the header or publish from a creator." />
        ) : (
          filtered.map((p: Record<string, unknown>) => (
            <GridListRow
              ...
              columns={[
                ...
                { key: 'Action', value: String(p.actionType ?? '-') },
              ]}
              onEdit={() => openEdit(p)}
            />
          ))
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editing ? 'Edit Public Power' : 'Add Public Power'}>
        <div className="space-y-4">
          <Input label="Name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Power name" />
