    ...
    <>
          ...
          <Chip
            ...
            onRemove={() => {
              ...
              if (exceptions.includes(t))
                set(
                  'armorExceptions',
                  exceptions.filter((x) => x !== t),
                );
            }}
          >
            {t}
            {exceptions.includes(t) ? ' ⚡' : ''}
          </Chip>
        ))}
      </div>
      <div className="mb-4 flex items-center gap-2">
        <input
          ...
          className="w-48 rounded border border-border-light bg-surface px-2 py-1 text-sm"
        />
        <button
          type="button"
          onClick={() => {
          ...
          className="flex items-center gap-1 text-xs text-primary-link-fg hover:text-primary-fg-hover"
        >
      ...
      <p className="mb-2 text-sm text-text-muted">
        These damage types bypass armor damage reduction. Click to toggle.
      </p>
      ...
      <FieldRow label="Note">
        <TextInput wide value={(data.note as string) ?? ''} onChange={(v) => set('note', v)} />
      </FieldRow>
    </>
  );
}
