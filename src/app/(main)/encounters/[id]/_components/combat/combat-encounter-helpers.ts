...
export function sortCombatantsForTurnOrder(combatants: Combatant[], round: number): Combatant[] {
...
export function orderCombatantsByInitiative(combatants: Combatant[]): Combatant[] {
  ...
  const allies = combatants.filter((c) => c.combatantType === 'ally').sort(sortByRollAndAcuity);
  const enemies = combatants.filter((c) => c.combatantType === 'enemy').sort(sortByRollAndAcuity);
...
export function buildManualCombatantsFromForm(form: NewCombatantForm): TrackedCombatant[] {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    combatants.push({
      ...
      isAlly: form.combatantType === 'ally' || form.combatantType === 'companion',
...
export function buildCampaignCharacterCombatant(
  ...
  return {
    ...
    currentHealth:
      (d.currentHealth as number | undefined) ?? data.health?.current ?? data.health?.max ?? 20,
    maxEnergy: data.energy?.max ?? 10,
    currentEnergy:
      (d.currentEnergy as number | undefined) ?? data.energy?.current ?? data.energy?.max ?? 10,
    armor: 0,
