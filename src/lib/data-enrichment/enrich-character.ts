...
export function enrichCharacterData(
  ...
  const publicEquipmentItems = publicLibraries?.items?.filter(
    (i) => itemTypeIs(i, 'equipment') || !String(i.type || '').trim(),
  );

  return {
    powers: enrichPowers(character.powers, userPowers, powerPartsDb || [], publicLibraries?.powers),
    techniques: enrichTechniques(
      character.techniques,
      userTechniques,
      techniquePartsDb || [],
      publicLibraries?.techniques,
    ),
    weapons: enrichItems(
