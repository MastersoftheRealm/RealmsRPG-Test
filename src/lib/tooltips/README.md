...
**Word-tied label tips (no Info icon):** use `WordHelpTip` + `getAbilityHelp` / `getDefenseHelp` from `tooltip-text.tsx` (ability/defense names on the sheet and guided creator — TASK-546). Same Floating UI engine as `InfoTippy`.
