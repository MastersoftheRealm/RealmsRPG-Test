...
| **mobile-audit.mjs** | Mobile UX audit: drives a touch-emulated Chromium (`pointer:coarse`, `hover:none`) over the app's routes at 390px + 360px and records horizontal overflow, clipped/spilling text, sub-44px and bloated touch targets, and overlapping controls. Writes screenshots + `findings.json`. | `node scripts/mobile-audit.mjs [--base http://localhost:3000] [--out reports/…] [--only <route-id>]`. Needs a running server. **Coarse pointer matters** — narrowing a desktop browser does not reproduce phone sizing. |
